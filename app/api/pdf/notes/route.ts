...
import { pdfNotes } from "@/lib/db/schema";
...
export async function GET(request: Request) {
  ...
  const where =
    pageNumber != null && Number.isFinite(pageNumber)
      ? and(eq(pdfNotes.fileName, fileName), eq(pdfNotes.pageNumber, pageNumber))
      : eq(pdfNotes.fileName, fileName);

  const rows = await db
    .select()
    .from(pdfNotes)
    .where(where)
    .orderBy(desc(pdfNotes.updatedAt))
    .limit(limit);
